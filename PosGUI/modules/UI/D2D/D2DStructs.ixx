...
export namespace PGUI::UI::D2D
{
	...
	[[nodiscard]] auto D2D1MatrixToMatrix3x2(D2D1_MATRIX_3X2_F matrix)
	{
		return Matrix3x2{ 
	...
	{
		public:
		constexpr MappedRect(D2D1_MAPPED_RECT mappedRect, UINT32 height) noexcept :
	...
	struct BitmapProperties : public D2D1_BITMAP_PROPERTIES1
	{
		...
		constexpr BitmapProperties(D2D1_PIXEL_FORMAT pixelFormat, std::pair<float, float> dpi, BitmapOptions bitmapOptions = BitmapOptions::None) noexcept :
			D2D1_BITMAP_PROPERTIES1{ .pixelFormat = pixelFormat, 
			.dpiX = dpi.first, .dpiY = dpi.second,
			.bitmapOptions = static_cast<D2D1_BITMAP_OPTIONS>(bitmapOptions) }
		{
		}

		explicit(false) constexpr BitmapProperties(D2D1_BITMAP_PROPERTIES1 bitmapProperties) noexcept :
			D2D1_BITMAP_PROPERTIES1{ bitmapProperties }
		{
		}
		explicit(false) constexpr BitmapProperties(D2D1_BITMAP_PROPERTIES bitmapProperties) noexcept :
			D2D1_BITMAP_PROPERTIES1{ .pixelFormat = bitmapProperties.pixelFormat,
			.dpiX = bitmapProperties.dpiX, .dpiY = bitmapProperties.dpiY,
		...
		{
			return D2D1_BITMAP_PROPERTIES1{ .pixelFormat = pixelFormat, .dpiX = dpiX, .dpiY = dpiY,
		...
		[[nodiscard]] constexpr auto GetBitmapOptions() const noexcept { return static_cast<BitmapOptions>(bitmapOptions); }
	};
	...
	struct BitmapBrushProperties : public D2D1_BITMAP_BRUSH_PROPERTIES
	{
		...
		{
			return D2D1_BITMAP_BRUSH_PROPERTIES{ .extendModeX = extendModeX,
				.extendModeY = extendModeY, .interpolationMode = interpolationMode };
		}
	};
	...
	struct BezierSegment : public D2D1_BEZIER_SEGMENT
	{
		...
		constexpr BezierSegment(PointF point1, PointF point2, PointF point3) noexcept :
	...
	struct ArcSegment : public D2D1_ARC_SEGMENT
	{
		...
		ArcSegment(PointF point, SizeF size, float rotationAngle, 
			...
			D2D1_ARC_SEGMENT{ .point = point, .size = size, .rotationAngle = rotationAngle,
		...
		{
			return D2D1_ARC_SEGMENT{ .point = point, .size = size, .rotationAngle = rotationAngle,
		...
		[[nodiscard]] constexpr auto GetSweepDirection() const noexcept { return static_cast<SweepDirection>(sweepDirection); }
